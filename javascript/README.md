...
カメラ選択でピクセルピッチ、レンズ選択で焦点距離が自動入力され、ピクセルスケールと推奨グリッドを自動計算します。手入力も可能。FITS ヘッダーからカメラ・レンズを自動認識し、型番も表示されます。
...
RA/DEC や焦点距離・ピクセルピッチのヒントを与えると、ソルブ速度が大幅に向上します。

1. **カメラ/レンズ** を選択 → 焦点距離・ピクセルピッチが自動入力される
...
| パラメータ | 説明 |
|-----------|------|
| Camera | カメラ機種。FITS ヘッダーの INSTRUME から自動認識。ピクセルピッチを自動入力。前回の選択も記憶される |
| Lens | レンズ/鏡筒。FITS ヘッダーの FOCALLEN から近似マッチ。焦点距離を自動入力。前回の選択も記憶される |
| Focal length | 焦点距離 (mm)。レンズ選択時は自動入力、手入力も可能 |
| Pixel pitch | ピクセルピッチ (μm)。カメラ選択時は自動入力、手入力も可能 |
...
- RA/DEC ヒントと焦点距離・ピクセルピッチを入力するとソルブ速度が劇的に向上する
- 天体名を入力して Search ボタンで座標を取得するのが最も簡単

### ソルブ失敗

- 焦点距離・ピクセルピッチが大幅に間違っていないか確認
- Scale Error を大きくする（30 → 50）（API モード時）
