...
function SolverParameters()
{
   ...
   this.load = function()
   {
      if ( Settings.has( KEY_PYTHON_PATH ) )
         this.pythonPath = Settings.readGlobal( KEY_PYTHON_PATH, DataType_String );
      if ( Settings.has( KEY_SCRIPT_DIR ) )
         this.scriptDir = Settings.readGlobal( KEY_SCRIPT_DIR, DataType_String );
      if ( Settings.has( KEY_GRID ) )
         this.grid = Settings.readGlobal( KEY_GRID, DataType_String );
      if ( Settings.has( KEY_OVERLAP ) )
         this.overlap = Settings.readGlobal( KEY_OVERLAP, DataType_Int32 );
   };

   // Settings APIに保存
   this.save = function()
   {
      Settings.writeGlobal( KEY_PYTHON_PATH, DataType_String, this.pythonPath );
      Settings.writeGlobal( KEY_SCRIPT_DIR,  DataType_String, this.scriptDir );
      Settings.writeGlobal( KEY_GRID,        DataType_String, this.grid );
      Settings.writeGlobal( KEY_OVERLAP,     DataType_Int32,  this.overlap );
   };
...
function SolverEngine()
{
   ...
   this.execute = function( inputPath, params )
   {
      ...
      var stdout = P.stdout.trim();
      var stderr = P.stderr.trim();
