...
from astap_solver import ASTAPSolver
...
def main():
    ...
    if not astap_path:
        logger.error("ASTAP executable path not specified. Use --astap-path or config file.")
        return 1
    ...
    try:
        ...
        for sf, result in zip(split_files, solve_results.values()):
            if result['success']:
