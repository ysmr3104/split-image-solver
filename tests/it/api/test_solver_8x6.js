#!/usr/bin/env node
/**
 * test_solver_8x6.js — IT-Solver API (8x6, 14mm rectilinear)
 *
 ...
 *   ASTROMETRY_API_KEY=xxx node tests/it/api/test_solver_8x6.js
 */
...
console.log("IT-Solver API (8x6, 14mm rectilinear)");
...
var pass = true;
// 8x6 は API 変動が大きいため ±2 の許容
if (successCount >= Math.max(totalTarget - 2, 1)) {
    console.log("PASS: 成功タイル数 >= " + Math.max(totalTarget - 2, 1));
} else {
    console.log("FAIL: 成功タイル数が不足 (" + successCount + " < " + Math.max(totalTarget - 2, 1) + ")");
    pass = false;
}
