#!/usr/bin/env node
/**
 * test_solver_2x2.js — IT-Solver API (2x2, 50mm rectilinear)
 *
 ...
 *   ASTROMETRY_API_KEY=xxx node tests/it/api/test_solver_2x2.js
 */
...
var HINTS_FIXTURE_FILE = path.join(__dirname, "../../fixtures/tile_hints_local_" + MODE + ".json");
...
console.log("IT-Solver API (2x2, 50mm rectilinear)");
...
targetTiles.forEach(function(ht) {
    ...
    // context に tile と hints を渡してソルブ
    ctx.TILE_SOLVE = tile;
...
var pass = true;
// 精密ヒント付きなので全タイル成功を期待 (最低 3/4)
if (successCount >= Math.max(totalTarget - 1, 1)) {
    console.log("PASS: 成功タイル数 >= " + Math.max(totalTarget - 1, 1));
} else {
    console.log("FAIL: 成功タイル数が不足 (" + successCount + " < " + Math.max(totalTarget - 1, 1) + ")");
    pass = false;
}
