#!/usr/bin/env node
/**
 * test_solver_equisolid_8x6.js — IT-Solver API (equisolid 8x6, Sigma 15mm fisheye)
 *
 ...
 *   ASTROMETRY_API_KEY=xxx node tests/it/api/test_solver_equisolid_8x6.js
 */
...
var HINTS_FIXTURE_FILE = path.join(__dirname, "../../fixtures/tile_hints_local_equisolid_8x6.json");
...
console.log("IT-Solver API (equisolid 8x6, Sigma 15mm fisheye)");
...
if (successCount >= Math.max(totalTarget - 4, 1)) {
    console.log("PASS: 成功タイル数 >= " + Math.max(totalTarget - 4, 1));
} else {
    console.log("FAIL: 成功タイル数が不足 (" + successCount + " < " + Math.max(totalTarget - 4, 1) + ")");
    pass = false;
}
