"""
test_solver_equisolid_8x6.py — IT-Solver (equisolid 8x6, Sigma 15mm fisheye)

wavefront を通さず、フィクスチャから事前定義したヒントで
equisolid 魚眼 8x6 タイルの per-tile ソルブ動作を確認する。
...
@pytest.mark.slow
def test_local_tile_solve_equisolid_8x6():
    """equisolid 8x6 グリッド: 精密ヒントで解けるべきタイル (12/48) が解けることを確認。

    フィクスチャの batch_success=True タイルのみソルブ対象。
    PixInsight wavefront で解けた 12 タイルが、per-tile でも同様に解けることを検証。
    """
    result = _run_tile_solve("equisolid_8x6", timeout_per_tile=240)
    _print_report(result)

    total = result["tiles_total"]
    solved = result["tiles_solved"]
    fixture = result["fixture"]
    batch_solved = fixture.get("batch_solved", 12)
    ...
    assert solved >= batch_solved - 2, (
        f"Expected ≥{batch_solved - 2}/{total} tiles solved with refined hints, "
        f"got {solved}/{total}. Failed: {failed}"
    )

    # 成功タイルの pixel_scale がメジアンスケールと整合すること
    median_scale = fixture.get("median_scale", 50.0)
