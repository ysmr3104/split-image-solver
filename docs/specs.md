...
- 下限クランプ: `SCALE_EXTRAPOLATE_MIN_MARGIN` (±10%)
...
- **座標整合性**: 期待座標からの乖離がタイル FOV × 0.5（最小 0.5°）超なら棄却
...
逸脱判定: 隣接ペアとの平均角度差 > `toleranceArcsec × 3`（デフォルト `toleranceArcsec=5.0` arcsec）
のタイルを無効化（`status="failed"`, `wcs=null`）し、WCS 統合の制御点から除外する
...
| `--dir <temp_dir>` | 出力先を一時ディレクトリに指定（入力 FITS を変更しない） |

**入力ファイルの保護:**

`--dir` フラグにより、solve-field が生成する一時ファイル (.wcs, .solved, .axy 等) は一時ディレクトリに出力されます。また `write_back_wcs=False` により、ソルブ結果の WCS ヘッダーを元の FITS に書き戻さないため、入力タイル FITS は一切変更されません。一時ディレクトリはソルブ完了後に自動削除されます。
...
Local モードも API / ImageSolver と同じく、`solverFn` が呼ばれるたびに1タイルを Python `--solve-single-tile` で逐次ソルブします。wavefront のヒント精緻化（成功タイルの WCS から次タイルの RA/DEC を改善）が全モードで統一的に機能します。

旧実装では Python 側で全タイルを一括バッチ実行 (Pass 1 並列ソルブ + Pass 2 WCS由来ヒントでリトライ) し、JS の `solverFn` は結果の辞書ルックアップのみ行っていました。この構造では wavefront が精緻化したヒントで再ソルブすることができませんでした。現在は Python 側のバッチモード・Pass 1/Pass 2・偽陽性フィルタは全て廃止され、偽陽性フィルタ（スケール比・座標乖離チェック）は JS 側 `solveWavefront()` が一元的に担当します。
...
|--------|---------|-------------|---------|
| 2x2 (rectilinear, 50mm) | 2×2 (4タイル) | 4/4 | ≥ 4/4 |
| 8x6 (rectilinear, 14mm) | 8×6 (48タイル) | 8/48 | ≥ 8/48 |
| equisolid_8x6 (Sigma 15mm) | 8×6 (48タイル) | 12/48 | ≥ 12/48 |
| equidistant_12x8 (AstrHori 6.5mm) | 12×8 (96タイル) | 12/96 | ≥ 12/96 |
...
4. **Local モードの Python 起動オーバーヘッド**: per-tile 呼び出しのため Python プロセスをタイル数分起動する（1回あたり ~500ms）。8x6 グリッドでは数分の追加オーバーヘッドとなるが、wavefront ヒント精緻化による成功率向上とのトレードオフ
