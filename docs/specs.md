...
Split Image Solver は、広角〜超広角の星野写真をタイルに分割し、astrometry.net API、ローカル solve-field、またはPixInsight内蔵 ImageSolver で各タイル（またはフル画像）をプレートソルブした後、個別の WCS（World Coordinate System）解を統合して元画像全体の WCS を生成するツールです。

PixInsight の ImageSolver では対応できない広角画像（FOV > 60°）を主な対象としており、フルサイズセンサー + 14mm レンズ（対角 FOV ~147°）のような超広角構成でも動作します。

**精度の順: Local > API > ImageSolver**。Split ソルブでは画像中心から離れるほど歪みが大きく、周辺の精度を重視する場合は [Manual Image Solver](https://github.com/ysmr3104/manual-image-solver) の利用を推奨します。本ツールは AnnotateImage の完璧な精度を求めるものではなく、ImageSolver 単体では不可能な超広角画像での SPCC 実行を可能にすることが主目的です。
...
Step 2: グリッド分割（NxM タイル + オーバーラップ + エッジスキップ）
Step 3: 1st パス — 各タイルをプレートソルブ（並列実行、スキップタイル除外）
...
| パラメータ | 説明 | デフォルト |
|-----------|------|-----------|
| `--grid ColsxRows` | グリッドパターン（例: 3x3, 8x6） | 3x3 |
| `--overlap` | タイル間のオーバーラップ (px) | 100 |
| `--skip-edges T,B,L,R` | エッジスキップ（上,下,左,右のタイル行/列数） | なし |
...
Tile solve grid (8x8):
  ...
  6    . . . . . . . .
  7    . . . . . . . .
  (O=solved, .=failed, -=skipped)
```

エッジスキップ使用時は、スキップされたタイルが `-` で表示されます。
...
| `--skip-edges` | str | エッジスキップ T,B,L,R（例: '0,3,0,0'） |
...
│   └── imagesolver_bridge.jsh           # PixInsight 内蔵 ImageSolver ブリッジ
├── config/
