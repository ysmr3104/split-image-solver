.PHONY: help setup install install-xisf test test-unit test-integration test-coverage lint format clean run-example check-solver
...
install-dev: setup install ## 開発環境をセットアップ（setup + install）
	...
	@echo ""
	@echo "📝 次のステップ:"
	@echo "  1. solve-field をインストール: brew install astrometry-net (macOS)"
	@echo "  2. 設定ファイルを作成: cp config/settings.example.json config/settings.json"
	@echo "  3. 星カタログ（インデックスファイル）をダウンロード"
	@echo "  4. make check-solver でsolve-fieldが使用可能か確認"
...
check-solver: ## solve-fieldのインストール状況を確認
	@echo "🔍 solve-fieldのインストール状況を確認しています..."
	@if command -v solve-field &> /dev/null; then \
		echo "✅ solve-field が見つかりました:"; \
		which solve-field; \
		solve-field --version 2>&1 | head -1 || echo "バージョン情報なし"; \
	else \
		echo "❌ solve-field が見つかりません"; \
		echo ""; \
		echo "インストール方法:"; \
		echo "  macOS: brew install astrometry-net"; \
		echo "  Linux: sudo apt-get install astrometry.net"; \
		echo "  詳細: docs/ASTROMETRY_NET_SETUP.md"; \
	fi
...
run-example: ## サンプル画像で実行（テスト用）
	...
	@echo "使用方法:"
	@echo "  $(PYTHON) python/main.py --input your_image.fits --output solved.fits --grid 3x3"
	@echo ""
	@echo "XISF形式の場合:"
	@echo "  $(PYTHON) python/main.py --input your_image.xisf --output solved.xisf --grid 3x3"
...
info: ## プロジェクト情報を表示
	...
	@if command -v solve-field &> /dev/null; then \
		echo "  ✅ solve-field: インストール済み"; \
	else \
		echo "  ❌ solve-field: 未インストール"; \
	fi
	@echo ""
