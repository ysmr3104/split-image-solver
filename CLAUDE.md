...
- **API モード**（デフォルト）: PJSR のみで動作。astrometry.net API + curl で通信。Python 不要
- **Local モード**: PJSR から ExternalProcess で Python `main.py` を呼び出し。ローカル solve-field でソルブ

### ファイル構成

```
PixInsight (PJSR)
├── SplitImageSolver.js    — メインスクリプト（UI + エンジン + Local モード統合）
│   ├── SolverSettingsDialog  — 設定ダイアログ（モード切替・API Key・Python 環境）
│   └── SplitSolverDialog     — メインダイアログ（機材・グリッド・座標・ソルブ実行）
...
└── equipment_data.jsh     — 機材 DB（カメラ・レンズ、型番付き）
```
...
2. ExternalProcess で Python `main.py` を実行（--grid, --overlap, --ra, --dec 等を引数で渡す）
3. stderr をリアルタイム表示（進捗ラベルにも反映）、Abort ボタン対応
4. 結果 JSON（--result-file）から wcs_keywords を取得
5. FITS キーワードを画像に直接適用 + regenerateAstrometricSolution()

### 主要モジュール

- **`javascript/SplitImageSolver.js`** — メインスクリプト。SolverSettingsDialog（設定ダイアログ）、SplitSolverDialog（メインUI）、タイル分割（splitImageToTiles）、タイルソルブ（solveMultipleTiles）、2パスリトライ（retryFailedTiles）、オーバーラップ検証（validateOverlap）、WCS 統合（mergeWcsSolutions）、WCS 適用（applyWCSToImage, setCustomControlPoints）、Local モード実行（doLocalSolve）を含む。
...
- **`javascript/equipment_data.jsh`** — 機材データベース（カメラ 66 機種 + レンズ/鏡筒 48 種）。カメラは instrume（型番）、レンズは model（型番）フィールド付き。
...
| `SplitImageSolver/apiKey` | String | astrometry.net API キー |
| `SplitImageSolver/pythonPath` | String | Python 実行ファイルパス |
| `SplitImageSolver/scriptDir` | String | split-image-solver リポジトリパス |
| `SplitImageSolver/pixelScale` | Double | 前回使用のピクセルスケール |
| `SplitImageSolver/camera` | String | 前回選択のカメラ名 |
| `SplitImageSolver/lens` | String | 前回選択のレンズ名 |
...
- **UI 初期化順序**: `updateScaleAndFov()` は `scaleEdit`、`fovInfoLabel`、`gridPresets`、`gridCombo` が全て定義された後に呼び出すこと。定義前に呼ぶと undefined エラーになる。
